<template>
	<button href="#" :class="{active:currentPage !== '' && currentPage===pageName}" class="focus:(outline-none shadow-focus) hover:bg-gray-100 flex flex-grow items-center px-[2.4rem] text-base ">
		<slot name="icon">
...
export default defineComponent({
	...
	computed: {
		getIcon() {
			if(this.pageName === ''){
				return this.icon
			} else if (this.iconExeption.includes(this.icon)){
				return this.icon
